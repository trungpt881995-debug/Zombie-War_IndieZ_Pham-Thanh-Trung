    ...
    public sealed class ControlFeatureTests
    {
        ...
        private sealed class FakeControlView : IControlView
        {
            ...
            public void Down(int id, float x, float y) =>
                PointerDownRequested?.Invoke(new ControlPointerSample(id, x, y));

            public void Drag(int id, float x, float y) =>
                PointerDragged?.Invoke(new ControlPointerSample(id, x, y));

            public void Up(int id) => PointerUpRequested?.Invoke(id);
            public void Cancel() => CancelRequested?.Invoke();
        }

        private static DynamicJoystickModel CreateModel(
            float deadZone = 0.2f,
            float maxRadius = 100f,
            float sensitivity = 1f)
        {
            return new DynamicJoystickModel(
                new JoystickSettings(deadZone, maxRadius, sensitivity));
        }
