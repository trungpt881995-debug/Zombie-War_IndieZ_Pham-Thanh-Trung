...
namespace ZombieWar.Composition.Projectile
{
    ...
    public sealed class HitscanProjectileLauncher : IProjectileLauncher
    {
        ...
        public bool TryLaunch(in ProjectileLaunchRequest request, out EntityId projectileId)
        {
            ...
            _events.Publish( new ProjectileLaunchedEvent(projectileId, request.OwnerId));
        ...
        private void FireSingle( EntityId projectileId, in ProjectileLaunchRequest request, Vector3 origin, Vector3 direction)
        {
            Vector3 rayOrigin = GetRayOrigin(origin, direction);
            float rayDistance = GetRayDistance(request.MaxRange);

            Vector3 endPoint = origin + direction * request.MaxRange;

            ProjectileEndReason endReason = ProjectileEndReason.MaxRangeReached;

            if (Physics.Raycast( rayOrigin, direction, out RaycastHit hit, rayDistance, _hitMask, _triggerInteraction))
            {
                endPoint = hit.point;
        ...
        private void FirePiercing(EntityId projectileId, in ProjectileLaunchRequest request, Vector3 origin, Vector3 direction)
        {
            Vector3 rayOrigin = GetRayOrigin(origin, direction);
            float rayDistance = GetRayDistance(request.MaxRange);
            ...
            int hitCount = Physics.RaycastNonAlloc(rayOrigin, direction, _raycastBuffer, rayDistance, _hitMask, _triggerInteraction);
        ...
        private void FireExplosion(EntityId projectileId, in ProjectileLaunchRequest request, Vector3 origin, Vector3 direction)
        {
            Vector3 rayOrigin = GetRayOrigin(origin, direction);
            float rayDistance = GetRayDistance(request.MaxRange);

            Vector3 explosionPoint = origin + direction * request.MaxRange;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (damageable == null || !_uniqueEntities.Add(damageable.EntityId))
                {
                    continue;
                }
        ...
        private void ApplyDamage(EntityId projectileId, in ProjectileLaunchRequest request, IDamageable damageable, Vector3 worldPoint, bool playHitFeedback)
        {
        ...
        private IDamageable FindDamageable(Collider collider)
        {
            ...
            collider.GetComponentsInParent(true, _behaviourBuffer);
        ...
        private Vector3 GetRayOrigin(Vector3 origin, Vector3 direction)
        {
        ...
        {
            return Mathf.Max(0.001f, maxRange - _muzzleForwardOffset);
        }

        private void PublishCompleted(EntityId projectileId, EntityId ownerId, ProjectileEndReason reason)
        {
            _events.Publish(
