...
namespace ZombieWar.Features.Weapon.Strategies
{
    ...
    {
        public const int PelletCount = 7;
        private readonly IWeaponProjectilePort _projectiles;
        private readonly IWeaponFeedbackPort _feedback;

        public ShotgunFireStrategy(IWeaponProjectilePort projectiles, IWeaponFeedbackPort feedback)
        ...
        public bool Fire(in WeaponDefinition weapon, in WeaponFireContext context)
        {
            ...
            if (!WeaponDirection.TryFromTo(in origin, in targetPoint, out WeaponDirection baseDirection))
                return false;

            float step = PelletCount > 1 ? weapon.SpreadAngle / (PelletCount - 1) : 0f;
            float first = -weapon.SpreadAngle * 0.5f;
            int launchedCount = 0;
            for (int i = 0; i < PelletCount; i++)
            {
                WeaponDirection direction = baseDirection.RotateYawDegrees(first + step * i);
                var request = new WeaponProjectileRequest(
                    context.OwnerId, WeaponProjectileProfileId.ShotgunPellet,
                    in origin, in direction,
                    weapon.ProjectileSpeed, weapon.Damage,
                    weapon.MaxRange, weapon.ProjectileLifetime,
                    in targetPoint, false, 0f);
                if (_projectiles.TryLaunch(in request)) launchedCount++;
            }
            if (launchedCount > 0) _feedback.OnShotFired(context.OwnerId, weapon.Type);
