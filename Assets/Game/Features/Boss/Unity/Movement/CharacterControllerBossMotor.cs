...
{
    [DisallowMultipleComponent][RequireComponent(typeof(CharacterController))] public sealed class CharacterControllerBossMotor : MonoBehaviour, IBossMotor
    {
        [SerializeField] private float gravity = - 20f;
        [SerializeField] private float groundedStickVelocity = - 2f;
        private CharacterController _controller;
        private bool _enabled = true;
        private float _verticalVelocity, _normalizedSpeed;
        ...
        public void MoveTowards(in BossPoint target, float speed, float dt)
        {
            if (!_enabled || dt <= 0f)
            {
                Stop();
            ...
            Vector3 cur = transform.position, delta = new Vector3(target.X - cur.x, 0f, target.Z - cur.z);
            Vector3 horizontal = Vector3.zero;
            if (delta.sqrMagnitude > 0.000001f && speed > 0f)
            {
            ...
            _controller.Move(horizontal * dt);
        }
        public void Stop()
        {
            _normalizedSpeed = 0f;
