...
namespace ZombieWar.Features.Boss.Unity.Movement
{
    ...
    public sealed class NavMeshBossMotor : MonoBehaviour, IBossMotor, IBossSteeringProvider
    {
        [Header("Agent Shape - tune per Boss prefab")]
        [Tooltip("Local-avoidance radius. For narrow passages, remember baked Agent Type radius also controls static-world clearance.")]
        ...
        [Tooltip("Lower number = higher avoidance priority. Boss defaults above Zombie crowd priority (Zombie baseline is 50).")]
        [SerializeField, Range(0, 99)] private int avoidancePriority = 20;

        [Tooltip("Small deterministic spread prevents two Bosses from having identical avoidance priority.")]
        [SerializeField, Range(0, 20)] private int avoidancePrioritySpread = 8;

        [Header("Path Recalculation")]
        [Tooltip("Minimum gameplay-time interval between path requests while chasing a moving Soldier.")]
        [SerializeField, Min(0.02f)] private float repathInterval = 0.1f;

        [Tooltip("Target must move at least this far before a normal repath is requested.")]
        [SerializeField, Min(0f)] private float destinationChangeThreshold = 0.2f;

        [Tooltip("Nearest NavMesh search radius around the Soldier target point.")]
        [SerializeField, Min(0.01f)] private float destinationSampleDistance = 2f;

        [Header("Spawn / Recovery")]
        [Tooltip("Nearest NavMesh search radius around the requested Boss spawn point.")]
        [SerializeField, Min(0.01f)] private float warpSampleDistance = 3f;

        [Tooltip("Small search radius used if an enabled Boss loses its NavMesh binding.")]
        [SerializeField, Min(0.01f)] private float recoverySampleDistance = 1.5f;
        ...
        public void Warp(in BossPoint position)
        {
            ...
            if (!TrySample(requested, warpSampleDistance, out Vector3 sampled))
            {
                ...
                WarnMissingNavMeshOnce(
                    $"Boss spawn position {requested} has no NavMesh within {warpSampleDistance:0.##}m.");
                return;
            }
            ...
            if (!_agent.isOnNavMesh)
            {
                SetAgentComponentEnabled(false);
                WarnMissingNavMeshOnce(
                    $"NavMeshAgent could not bind Boss at sampled spawn position {sampled}.");
                return;
            }
        ...
        public void SetEnabled(bool enabled)
        {
            ...
            {
                WarnMissingNavMeshOnce(
                    "Boss NavMeshAgent cannot be enabled because no valid baked NavMesh is available near the Boss.");
                return;
            }
        ...
        public void MoveTowards(in BossPoint target, float speed, float deltaTime)
        {
            ...
            {
                WarnMissingNavMeshOnce(
                    "Boss cannot chase because its NavMeshAgent is not bound to a valid NavMesh.");
                return;
            }
            ...
            if (!_hasRequestedDestination ||
                pathNeedsRepair ||
                (intervalElapsed && targetMovedEnough))
            {
