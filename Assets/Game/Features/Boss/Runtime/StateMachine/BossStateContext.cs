...
namespace ZombieWar.Features.Boss.StateMachine
{
    public sealed class BossStateContext
    {
        ...
        public IBossView View
        {
            get;
        }
        public IBossMotor Motor
        {
            get;
        }
        public IBossTargetProvider TargetProvider
        {
            get;
        }
        ...
        public IBossTargetRegistrationPort TargetRegistration
        {
            get;
        }
        public IBossPoolReturnPort PoolReturn
        {
            get;
        }
        public IBossFeedbackPort Feedback
        {
            get;
        }
        ...
        public Action < BossStateId > ChangeState
        {
            get;
        }
        public BossStateContext(BossModel model, IBossView view, IBossMotor motor, IBossTargetProvider targetProvider, IBossAttackStrategy attackStrategy,
        IBossTargetRegistrationPort targetRegistration, IBossPoolReturnPort poolReturn, IBossFeedbackPort feedback, IEventBus eventBus,
        Action < BossStateId > changeState)
        {
            ...
            TargetRegistration = targetRegistration ?? throw new ArgumentNullException(nameof(targetRegistration));
            PoolReturn = poolReturn ?? throw new ArgumentNullException(nameof(poolReturn));
            Feedback = feedback ?? NullBossFeedbackPort.Instance;
        ...
        public bool TryRefreshOrAcquireTarget(out BossTarget target)
        {
            ...
            BossPoint p = Motor.Position;
            if (TargetProvider.TryAcquireTarget(in p, out target) && target.IsValid)
            {
                Model.SetTarget(in target);
                return true;
            }
