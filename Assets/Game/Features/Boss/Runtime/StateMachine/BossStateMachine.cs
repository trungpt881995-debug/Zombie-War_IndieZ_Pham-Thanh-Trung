...
namespace ZombieWar.Features.Boss.StateMachine
{
    ...
    {
        private readonly Dictionary < BossStateId, IBossState > _states = new Dictionary < BossStateId, IBossState > (5);
        private IBossState _current;
        ...
        public void Change(BossStateId id)
        {
            if (_current != null && _current.Id == id) return;
            if (!_states.TryGetValue(id, out IBossState next)) throw new InvalidOperationException($"Boss state is not registered: {id}");
            _current?.Exit();
            _current = next;
            _current.Enter();
        }
