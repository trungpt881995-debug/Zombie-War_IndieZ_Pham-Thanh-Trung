...
namespace ZombieWar.Features.Boss.Domain
{
    public readonly struct BossDefinition
    {
        ...
        public float AttackExitRangeBonus
        {
            get;
        }
        ...
        public float AiDecisionInterval
        {
            get;
        }
        ...
        public float HitReactionMinInterval
        {
            get;
        }
        public float AttackAnimationTimeout
        {
            get;
        }
        ...
        float attackExitRangeBonus, float attackCooldown, float aiDecisionInterval, float spawnDuration, float hitReactionDuration,
        float hitReactionMinInterval, float attackAnimationTimeout, float deathDuration, float scale, int rewardScore, float spawnOffsetX,
        float spawnOffsetY, float spawnOffsetZ, BossAttackType attackType = BossAttackType.BasicMelee)
        {
            if (id == BossId.None) throw new ArgumentOutOfRangeException(nameof(id));
            ...
            if (rewardScore < 0) throw new ArgumentOutOfRangeException(nameof(rewardScore));
            Id = id;
            MaxHealth = maxHealth;
            MoveSpeed = moveSpeed;
