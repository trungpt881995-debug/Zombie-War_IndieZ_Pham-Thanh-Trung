...
namespace ZombieWar.Features.Boss.Domain
{
    public readonly struct BossPoint : IEquatable < BossPoint >
    {
        ...
        public BossPoint(float x, float y, float z)
        {
            if (float.IsNaN(x) || float.IsNaN(y) || float.IsNaN(z) || float.IsInfinity(x) || float.IsInfinity(y) || float.IsInfinity(z)) throw new ArgumentOutOfRangeException(nameof(x));
            X = x;
            Y = y;
            Z = z;
        }
        public BossPoint Add(float x, float y, float z) => new BossPoint(X + x, Y + y, Z + z);
        public static float SqrDistanceXZ(in BossPoint a, in BossPoint b)
        {
