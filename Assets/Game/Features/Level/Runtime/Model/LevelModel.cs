...
namespace ZombieWar.Features.Level.Model
{
    public sealed class LevelModel
    {
        ...
        public void SetReady()
        {
            ...
            Phase = LevelPhase.None;
            ProgressionEnabled = false;
        }
        public void Begin(LevelDefinition d)
        {
            GameLevel = d.Id;
            SoldierGroupLevel = SoldierGroupLevelId.Level1;
            NormalZombieKillCount = 0;
            RequiredBossObjectives = d.RequiredBossObjectives;
            DefeatedBossObjectives = LevelBossObjectiveId.None;
            State = LevelState.Running;
            Phase = LevelPhase.NormalCombat;
            ProgressionEnabled = true;
        }
        public void AddKills(int count)
        {
            NormalZombieKillCount += count;
        }
        public void SetSoldierGroupLevel(SoldierGroupLevelId level)
        {
            SoldierGroupLevel = level;
        }
        public void StartBossPhase()
        {
            Phase = LevelPhase.BossPhase;
        }
        public void SetProgressionEnabled(bool enabled)
        {
            ProgressionEnabled = enabled && State == LevelState.Running;
        }
        public void AddDefeatedBoss(LevelBossObjectiveId boss)
        {
            DefeatedBossObjectives |= boss;
        }
        ...
        public void Reset()
        {
            State = LevelState.Ready;
            Phase = LevelPhase.None;
            GameLevel = GameLevelId.None;
            SoldierGroupLevel = SoldierGroupLevelId.Level1;
            NormalZombieKillCount = 0;
            ProgressionEnabled = false;
            RequiredBossObjectives = LevelBossObjectiveId.None;
            DefeatedBossObjectives = LevelBossObjectiveId.None;
        }
    }
}
