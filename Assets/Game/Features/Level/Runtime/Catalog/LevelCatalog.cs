using System; using System.Collections.Generic; using ZombieWar.Features.Level.Domain;
namespace ZombieWar.Features.Level.Catalog
{
    public sealed class LevelCatalog : ILevelCatalog
    {
        ...
        {
            if(definitions==null||definitions.Length==0) throw new ArgumentException("At least one level definition is required.",nameof(definitions));
            _levels=new Dictionary<GameLevelId,LevelDefinition>(definitions.Length); int finalCount=0;
        ...
        {
            definition=null; if(id==GameLevelId.None)return false; return _levels.TryGetValue(id,out definition);
        }
    }
}
