...
namespace ZombieWar.Features.UI.Presentation
{
    public sealed class HealthPresenter
    {
        ...
        public HealthPresenter(GameplayHudModel model) => _model = model;
        ...
        public void Present(float normalized, float current, float max)
        {
            _model.HealthNormalized = Clamp(normalized);
            _model.CurrentHealth = current < 0 ? 0 : current;
            _model.MaxHealth = max < 0 ? 0 : max;
            Render();
        }
        private void Render() => _view?.SetHealth(_model.HealthNormalized, _model.CurrentHealth, _model.MaxHealth);
