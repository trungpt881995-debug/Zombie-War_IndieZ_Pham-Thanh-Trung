...
namespace ZombieWar.Features.UI.Controller
{
    public sealed class PauseController
    {
        ...
        private readonly SingleExecutionGuard _nav = new SingleExecutionGuard();
        private IPauseView _view;
        public PauseController(IUIFlowActionPort flow, IGameplayPausePort pause)
        {
            _flow = flow ?? throw new ArgumentNullException(nameof(flow));
            _pause = pause ?? throw new ArgumentNullException(nameof(pause));
        }
        public void Bind(IPauseView v)
        {
            Unbind();
        ...
        public void Reset() => _nav.Reset();
        private void Replay()
        {
            if (_nav.TryEnter()) _flow.Replay();
        }
        private void Continue() => _pause.Resume();
        private void Menu()
        {
            if (_nav.TryEnter()) _flow.Menu();
        }
    }
}
