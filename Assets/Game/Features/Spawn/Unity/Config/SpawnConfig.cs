...
namespace ZombieWar.Features.Spawn.Unity.Config
{
    ...
    public sealed class SpawnConfig : ScriptableObject
    {
        ...
        {
            [Min(1)] public int gameLevel; 
            [Range(1,4)] public int soldierGroupLevel; 
            [Min(1)] public int maxAlive; 
            [Min(0.01f)] public float interval; 
            [Min(1)] public int batchMin; 
            [Min(1)] public int batchMax;
            public SpawnTuningEntry Build()
            {
                var key=new SpawnDifficultyKey(gameLevel,soldierGroupLevel);
                var t=new SpawnTuning(maxAlive,interval,batchMin,batchMax);
                return new SpawnTuningEntry(in key,in t);
            }
        }
        ...
        public bool StartOnInitialize=>startOnInitialize; 
        ...
        public SpawnTuningCatalog BuildCatalog()
        {
            ...
            throw new InvalidOperationException("SpawnConfig requires tuning profiles.");

            var entries=new SpawnTuningEntry[profiles.Length]; 

            for(int i=0;i<profiles.Length;i++) 
            entries[i]=profiles[i].Build(); 

            return new SpawnTuningCatalog(entries);
        }
        private void Reset()
        {
