    ...
    public sealed class SpawnTuningCatalog : ISpawnTuningCatalog
    {
        ...
        public SpawnTuningCatalog(IReadOnlyList<SpawnTuningEntry> entries)
        {
            ...
            for(int i=0;i<entries.Count;i++)
            {
                ...
                throw new ArgumentException($"Duplicate spawn tuning key: {e.Key}.",nameof(entries));

                _items.Add(e.Key,e.Tuning);
            }
        }
        public bool TryGet(in SpawnDifficultyKey key,out SpawnTuning tuning) 
        { 
            tuning=default; 
            return _items.TryGetValue(key,out tuning); 
        }
    }
}
