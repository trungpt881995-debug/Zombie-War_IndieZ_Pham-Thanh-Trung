...
namespace ZombieWar.Features.Spawn.Model
{
    public sealed class SpawnModel
    {
        ...
        public void Initialize(in SpawnDifficultyKey key,in SpawnTuning tuning) 
        { 
            Difficulty=key; 
            Tuning=tuning; 
            Elapsed=0f; 
            StopReason=SpawnStopReason.None; 
            LastBatch=default; 
            SuccessfulSpawnCount=0; 
            RejectedSpawnCount=0; 
            State=SpawnState.Ready; 
        }
        public void SetDifficulty(in SpawnDifficultyKey key,in SpawnTuning tuning) 
        { 
            Difficulty=key; 
            Tuning=tuning; 
            Elapsed=0f; 
        ...
        { 
            ...
            State=SpawnState.Suspended; 
        }
        public void Resume() 
        { 
            if(State==SpawnState.Suspended) 
            State=SpawnState.Running; 
        }
        ...
        public void RecordBatch(in SpawnBatchResult result) 
        { 
            LastBatch=result; 
            SuccessfulSpawnCount += result.Spawned;

            if(result.Spawned < result.CapacityLimited) 
            RejectedSpawnCount += result.CapacityLimited-result.Spawned; 
        }
        public void Reset() 
        { 
            State=SpawnState.Uninitialized; 
            Difficulty=default; 
            Tuning=default; 
            Elapsed=0f; 
            StopReason=SpawnStopReason.None; 
