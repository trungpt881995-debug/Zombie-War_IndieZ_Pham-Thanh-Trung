...
namespace ZombieWar.Features.Spawn.Domain
{
    public readonly struct SpawnTuning : IEquatable<SpawnTuning>
    {
        ...
        public SpawnTuning(int maxAlive,float interval,int batchMin,int batchMax)
        {
            ...
            if (float.IsNaN(interval) || float.IsInfinity(interval) || interval <= 0f) throw new ArgumentOutOfRangeException(nameof(interval));
            if (batchMin <= 0) throw new ArgumentOutOfRangeException(nameof(batchMin));
            if (batchMax < batchMin) throw new ArgumentOutOfRangeException(nameof(batchMax));
            if (batchMax > maxAlive) throw new ArgumentOutOfRangeException(nameof(batchMax));
            MaxAlive=maxAlive; Interval=interval; BatchMin=batchMin; BatchMax=batchMax;
        }
