...
namespace ZombieWar.Features.VFX.Services
{
    public sealed class VFXRuntime : IVFXRuntime, IVFXRuntimeConfigurator
    {
        ...
        public VFXRuntime(VFXController controller) => _controller = controller ?? throw new ArgumentNullException(nameof(controller));
        ...
        public VFXHandle Play(in VFXRequest request) => _controller.Play(in request);
        public bool Stop(VFXHandle handle) => _controller.Stop(handle);
        public void SetMode(VFXGameplayMode mode) => _controller.SetMode(mode);
        public void Tick(float deltaTime) => _controller.Tick(deltaTime);
        public void CancelAll() => _controller.CancelAll();
        public void Initialize(IVFXCatalog catalog, IVFXPoolRegistry pools) => _controller.Initialize(catalog, pools);
