...
namespace ZombieWar.Features.Soldier.View
{
        ...
        ISoldierView
    {
        ...
        public void SetLocalFormationPosition(in SoldierPoint localPosition)
        {
            CachedTransform.localPosition = new Vector3(localPosition.X, localPosition.Y, localPosition.Z);
        }
        ...
        public void SetMovementSpeed(float normalizedSpeed)
        {
            ...
            animator.SetFloat(_movementSpeedHash,Mathf.Clamp01(normalizedSpeed));
        }

        public void SetAimDirection(in SoldierDirection direction, float rotationDegreesPerSecond, float deltaTime)
        {
            if (animator != null)
            {
                Vector3 worldDirection = new Vector3(direction.X, direction.Y, direction.Z);

                Vector3 localDirection = CachedTransform.InverseTransformDirection(worldDirection);

                animator.SetBool(_hasTargetHash, true);

                animator.SetFloat(_aimXHash, localDirection.x);

                animator.SetFloat(_aimYHash, localDirection.z);
            }

            if (aimPivot == null || !direction.HasDirection)
            {
                return;
            }
            ...
            aimPivot.rotation = Quaternion.RotateTowards(aimPivot.rotation,targetRotation, rotationDegreesPerSecond * deltaTime);
        }
